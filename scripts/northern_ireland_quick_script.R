...
# Final figures for NORTHERN IRELAND
...
dta %>% 
  ...
  levelplot(
    ...
    main="Northern Ireland, Mortality Rates (Log10 scale)",
    labels=list(cex=1.2),
    col="black",
    scales=list(
      x=list(cex=1.4), 
...
dta_rUK <- dta %>% 
  filter(
    country_code %in% c("GBRCENW", "GBR_SCO"),
    year >= 1950 & year <= 2010
  ) %>% 
  group_by(
    sex, year, age
...
tmp1 <- dta_nir %>% 
  select(year, age, sex, `Northern Ireland` = lg_death_rate)  

tmp2 <- dta_rUK  %>% 
  select(year, age, sex, `Rest of UK` = lg_death_rate) 

dif_nir_rest_UK <- tmp1 %>% 
  left_join(tmp2) %>% 
  mutate(
    `NI less Rest of UK` = `Northern Ireland` - `Rest of UK`
  ) %>% 
  select(
    year, age, sex, `NI less Rest of UK`
  )
...
tmp2 <- dta_we_less_nir  %>% 
  select(year, age, sex, `Rest of WE` = lg_death_rate) 

dif_nir_rest_WE <- tmp1 %>% 
  left_join(tmp2) %>% 
  mutate(
    `NI less Rest of WE` = `Northern Ireland` - `Rest of WE`
  ) %>% 
  select(
    year, age, sex, `NI less Rest of WE`
  )
...
comparisons <- dif_nir_rest_UK %>% 
  left_join(dif_nir_rest_WE) %>% 
