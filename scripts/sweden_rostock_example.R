#Sweden Quick Analysis for Rostock Retreat 



# Final figures for Sweden

# The final figures should be 

# 1) SCP of Sweden, 1900 to 2010
# 2) CLP Lattice plot
#   i ) Sweden - rNE
#  ii ) Sweden - rWE
#  iii) rNE - rWE
...
# Northern Ireland SCP - qual colour scheme
png(filename="figures/rostock/figure_01_lp.png", 
...
dta %>% 
  ...
  levelplot(
    ...
    main="Sweden, ages 0-90, Mortality Rates (Log10 scale)",
    labels=list(cex=1.2),
    col="black",
    scales=list(
      x=list(cex=1.4), 
...
# Figure 2 : composite: CLP Swe - rNE, Swe - WE, NE - WEm NE - SE, swe - SE---------------
...
dta_ne <- dta %>% 
  filter(
    country_code %in% c("DNK", "FIN", "SWE", "NOR", "ISL"),
    year >= 1950 & year <= 2010
  ) %>% 
  group_by(
    sex, year, age
...
# Difference between swe and rne
tmp1 <- dta_swe %>% 
  select(year, age, sex, `Sweden` = lg_death_rate)  

tmp2 <- dta_rne  %>% 
  select(year, age, sex, `Rest of NE` = lg_death_rate) 

dif_swe_rne <- tmp1 %>% 
  left_join(tmp2) %>% 
  mutate(
    `Swe less Rest of NE` = `Sweden` - `Rest of NE`
  ) %>% 
  select(
    year, age, sex, `Swe less Rest of NE`
  )

rm(tmp1, tmp2)


# Difference between ne and we

tmp1 <- dta_ne %>% 
  select(year, age, sex, `N. Europe` = lg_death_rate) 

tmp2 <- dta_we  %>% 
  select(year, age, sex, `W. Europe` = lg_death_rate) 

dif_ne_we <- tmp1 %>% 
  left_join(tmp2) %>% 
  mutate(
    `NE less WE` = `N. Europe` - `W. Europe`
  ) %>% 
  select(
    year, age, sex, `NE less WE`
  )
...
dif_swe_we <- tmp1 %>% 
  left_join(tmp2) %>% 
  mutate(
    `Swe less WE` = `Sweden` - `W. Europe`
  ) %>% 
  select(
    year, age, sex, `Swe less WE`
  )
...
tmp2 <- dta_se  %>% 
  select(year, age, sex, `S. Europe` = lg_death_rate) 

dif_ne_se <- tmp1 %>% 
  left_join(tmp2) %>% 
  mutate(
    `NE less SE` = `N. Europe` - `S. Europe`
  ) %>% 
  select(
    year, age, sex, `NE less SE`
  )
...
dif_swe_se <- tmp1 %>% 
  left_join(tmp2) %>% 
  mutate(
    `Swe less SE` = `Sweden` - `S. Europe`
  ) %>% 
  select(
    year, age, sex, `Swe less SE`
  )
...
comparisons <- dif_swe_rne %>% 
  left_join(dif_ne_we) %>% 
  left_join(dif_swe_we) %>% 
  left_join(dif_swe_se) %>% 
  left_join(dif_ne_se) %>% 
